...
if hasattr(User, 'REQUIRED_FIELDS'):
    ...
    if not (User.USERNAME_FIELD == 'mobile_number' or 'mobile_number' in User.REQUIRED_FIELDS):
        raise ImproperlyConfigured(
            "MobileBackend: Your User model must have an mobile number"
...
class MobileBackend(ModelBackend):
    ...
    def _authenticate(self, request, mobile_number=None, password=None, *args, **kwargs):
        matching_users = User.objects.filter(mobile_number__iexact=mobile_number)
        authenticated_users = [
            user for user in matching_users if (user.check_password(password) and self.user_can_authenticate(user))]
        if len(authenticated_users) == 1:
            # Happy path
            ...
            # either.
            raise User.MultipleObjectsReturned(
                "There are multiple users with the given mobile_number and "
                "password")
        return None
